...
class DepthSampler(nn.Module):
    ...
    def forward(self, xy, depth, cam2world, intersection_net, intrinsics):
        ...
        intersections = geometry.world_from_xy_depth(xy=xy,
                                                     depth=depth,
                                                     cam2world=cam2world,
...
class Raymarcher(nn.Module):
    ...
    def forward(self, cam2world, phi, uv, intrinsics):
        ...
        initial_depth = torch.zeros(
            (batch_size, num_samples, 1)).normal_(mean=0.05,
                                                  std=5e-4).to(device)
        ...
        for step in range(self.steps):
            ...
            signed_distance = self.out_layer(state[0]).view(
            ...
            if self.training:
                print("Raymarch step %d: Min depth %0.6f, max depth %0.6f" %
                      (step, depths[-1].min().detach().cpu().numpy(),
                       depths[-1].max().detach().cpu().numpy()))
        ...
        if not self.counter % 100:
            ...
            log.append(('image', 'raycast_progress',
                        torch.clamp(
                            torchvision.utils.make_grid(drawing_depths,
                                                        scale_each=False,
                                                        normalize=True), 0.0,
                            5), 100))
            ...
            fig = util.show_images([
                util.lin2img(signed_distance)[
                    i, :, :, :].detach().cpu().numpy().squeeze()
...
class DeepvoxelsRenderer(nn.Module):
    ...
    def build_net(self):
        ...
        if self.num_upsampling > 0:
            self.net += [
                pytorch_prototyping.UpsamplingNet(
                    per_layer_out_ch=self.num_upsampling * [self.nf0],
                    in_channels=4 * self.nf0,
                    upsampling_mode='transpose',
                    use_dropout=True,
                    dropout_prob=0.1),
                pytorch_prototyping.Conv2dSame(self.nf0,
                                               out_channels=self.nf0 // 2,
                                               kernel_size=3,
                                               bias=False),
    ...
    def forward(self, input):
        ...
        input = input.permute(0, 2,
                              1).view(batch_size, ch, self.img_sidelength,
                                      self.img_sidelength)
        out = self.net(input)
