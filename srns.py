...
class SRNsModel(nn.Module):
    def __init__(self,
        ...
        if self.fit_single_srn:  # Fit a single scene with a single SRN (no hypernetworks)
            ...
            self.latent_codes = nn.Embedding(num_instances,
                                             latent_dim).to(device)
            nn.init.normal_(self.latent_codes.weight, mean=0, std=0.01)

            self.hyper_phi = hyperlayers.HyperFC(
                hyper_in_ch=self.latent_dim,
        ...
        self.ray_marcher = custom_layers.Raymarcher(
    ...
    def get_comparisons(self, model_input, prediction, ground_truth=None):
        ...
        uv = model_input["uv"].to(device).float()
    ...
    def write_updates(self,
        ...
        if not iter % 100:
            ...
            rgb_loss = ((predictions.float().to(device) -
                         trgt_imgs.float().to(device))**2).mean(dim=2,
                                                                keepdim=True)
    ...
    def forward(self, input, z=None):
        ...
        # Parse model input.
        instance_idcs = input["instance_idx"].long().to(device)
        pose = input["pose"].to(device)
        intrinsics = input["intrinsics"].to(device)
        uv = input["uv"].to(device).float()
